    ...
    <div className="min-h-screen bg-gradient-to-br from-social-cream-100 via-social-cream-200 to-social-cream-300 py-6">
      <div className="max-w-6xl mx-auto px-4 space-y-5">
        <header className="rounded-3xl social-panel p-6 shadow-xl">
          ...
          <h1 className="text-3xl font-bold text-social-navy-700">Friends</h1>
          <p className="mt-2 text-social-navy-500">Connect with people you know and expand your network.</p>
          <div className="mt-3 text-sm text-social-navy-400">Signed in as {user?.username ?? "guest"}</div>
        </header>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <section className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl social-panel p-5 shadow">
              <header className="flex items-center justify-between">
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-social-navy-700">People you may know</h2>
                </div>
              </header>
                ...
                {recommendations.map((rec) => (
                  <div key={rec.id} className="rounded-xl bg-social-cream-200 border border-social-cream-400 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-social-navy-700">{rec.name}</p>
                        <p className="text-xs text-social-navy-400">{rec.status}</p>
                      </div>
                    </div>
                    <p className="mt-1 text-xs text-social-navy-500">{rec.mutual} mutual friends</p>
            ...
            <div className="rounded-2xl social-panel p-5 shadow">
              ...
              <h2 className="text-xl font-semibold text-social-navy-700">Friend Requests</h2>
              <div className="mt-3 space-y-3">
                {incoming.map((req) => (
                  <div key={req.id} className="flex items-center justify-between rounded-xl bg-social-cream-200 border border-social-cream-400 px-3 py-2">
                    <div>
                      <p className="font-semibold text-social-navy-700">{req.name}</p>
                      <p className="text-xs text-social-navy-400">{req.note}</p>
                    </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl social-panel p-4 shadow">
              ...
              <h3 className="text-lg font-semibold text-social-navy-700">Online Friends</h3>
              <ul className="mt-3 space-y-2 text-sm text-social-navy-600">
                <li className="flex justify-between">
                  <span>Close Friends</span>
                  <span className="rounded-full bg-social-cream-300 border border-social-gold-400 px-2 py-0.5 text-social-navy-600">12 online</span>
                </li>
                <li className="flex justify-between">
                  <span>All Friends</span>
                  <span className="rounded-full bg-social-cream-300 border border-social-cream-400 px-2 py-0.5 text-social-navy-500">4 away</span>
                </li>
              </ul>
