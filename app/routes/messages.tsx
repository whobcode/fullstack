    ...
    <div className="min-h-screen bg-gradient-to-br from-social-cream-100 via-social-cream-200 to-social-cream-300 py-6">
      <div className="max-w-6xl mx-auto px-4 space-y-5">
        <header className="rounded-3xl social-panel p-6 shadow-xl">
          ...
          <h1 className="text-3xl font-bold text-social-navy-700">Messages</h1>
          <p className="mt-2 text-social-navy-500">Stay in touch with friends and groups.</p>
          <div className="mt-2 text-sm text-social-navy-400">Logged in as {user?.username ?? "guest"}</div>
        </header>

        <div className="grid gap-6 lg:grid-cols-3">
          <section className="lg:col-span-2 space-y-4">
            <div className="rounded-2xl social-panel p-5 shadow">
              <h2 className="text-xl font-semibold text-social-navy-700">Conversations</h2>
              <div className="mt-3 space-y-3">
                {threads.map((thread) => (
                  <div key={thread.id} className="flex items-center justify-between rounded-xl bg-social-cream-200 border border-social-cream-400 px-4 py-3 hover:bg-social-cream-300 transition-colors cursor-pointer">
                    <div>
                      <p className="font-semibold text-social-navy-700">{thread.name}</p>
                      <p className="text-xs text-social-navy-400">{thread.preview}</p>
                    </div>
                    {thread.unread > 0 && (
            ...
            <div className="rounded-2xl social-panel p-5 shadow">
              <h2 className="text-lg font-semibold text-social-navy-700">Start a New Conversation</h2>
              <form className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="space-y-1 text-sm text-social-navy-600">
                  <span>To</span>
                ...
                <label className="space-y-1 text-sm text-social-navy-600 sm:col-span-2">
                  <span>Message</span>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl social-panel p-4 shadow">
              ...
              <p className="mt-1 text-sm text-social-navy-500">
